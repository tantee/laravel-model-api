...
class LaravelModelApi
{
                ...
                if ($validator->fails()) {
                    foreach ($validator->errors()->getMessages() as $key => $value) {
                        foreach ($value as $message) {
                            array_push($errorTexts, ["errorText" => $message,"field" => $key,"rows" => $rows]);
                        }
                    }
                    $success = false;
                }
        ...
        if ($success) {
          ...
          '*.'.$keyField => 'required',
                ...
                if ($success) {
                    $tempData = [];
                    foreach ($data as $row) {
                        if (! $override && isset($row['updated_at']) && ! self::isCurrentRecord($model, $row[$keyField], $row['updated_at'])) {
                            $success = false;
        ...
        if ($success && ! empty($validatorRule)) {
            ...
            foreach ($data as $row) {
                ...
                if ($validator->fails()) {
                    foreach ($validator->errors()->getMessages() as $key => $value) {
                        foreach ($value as $message) {
                            array_push($errorTexts, ["errorText" => $message,"field" => $key]);
                        }
                    }
                    $success = false;
                }
    ...
    public static function createModelByRequest(Request $request, $model, $validatorRule = [], $fillable = [])
    {
      ...
      'data' => 'required|array',
        ...
        if ($success) {
            $return = self::createModel($request->data, $model, $validatorRule, $fillable, false, (isset($request->with)) ? $request->with : null);
            $success = $return["success"];
            $errorTexts = $return["errorTexts"];
            $returnModels = $return["returnModels"];
        }

        return new ExtendedResourceCollection(collect($returnModels), $success, $errorTexts);
    }
    ...
    public static function replaceModelByRequest(Request $request, $model, $validatorRule = [], $fillable = [])
    {
      ...
      'data' => 'required|array',
        ...
        if ($success) {
            $return = self::replaceModel($request->data, $model, $validatorRule, $fillable, false, (isset($request->with)) ? $request->with : null);
            $success = $return["success"];
            $errorTexts = $return["errorTexts"];
            $returnModels = $return["returnModels"];
        }
    ...
    public static function readModelByRequest(Request $request, $model)
    {
        ...
        if ($success) {
            try {
                ...
                if (isset($request->key)) {
                    $returnModels = $returnModels->find($request->key);
                }

                if (isset($request->scope)) {
                    if (isset($request->scopeParam)) {
                        $returnModels = $returnModels->{$request->scope}($request->scopeParam);
                    } else {
                        $returnModels = $returnModels->{$request->scope}();
                    }
                }

                if (isset($request->with)) {
                    $returnModels = $returnModels->with($request->with);
                }
                ...
                if (! isset($request->key)) {
                    ...
                    if (isset($request->perPage) && is_numeric($request->perPage)) {
                        ...
                        if (isset($request->orderBy)) {
                            $returnModels->appends(['orderBy' => $request->orderBy]);
                        }
                        if (isset($request->with)) {
                            $returnModels->appends(['with' => $request->with]);
                        }
                        if (isset($request->scope)) {
                            $returnModels->appends(['scope' => $request->scope]);
                        }
                        if (isset($request->scopeParam)) {
                            $returnModels->appends(['scope' => $request->scopeParam]);
                        }
                    } else {
        ...
        return LaravelGenericApi::resultToResource(["success" => $success, "errorTexts" => $errorTexts, "returnModels" => $returnModels]);
    }
                    ...
                    if (empty($searchModel->getConnectionName()) || $searchModel->getConnectionName() == env('DB_CONNECTION', 'mysql')) {
                        ...
                        if (method_exists($searchModel, 'scopeActive')) {
                            ...
                            if (isset($request->data['scope'])) {
                                if (isset($request->data['scopeParam'])) {
                                    $searchModel = $searchModel->{$request->data['scope']}($request->data['scopeParam']);
                                } else {
                                    $searchModel = $searchModel->{$request->data['scope']}();
                                }
                            }
                        ...
                        $searchModel = $searchModel->where(function ($query) use ($searchField, $request) {
                            foreach ($searchField as $field) {
                                $query = $query->orWhere($field, 'LIKE', '%'.$request->data['keyword'].'%');
                            }
                        });
      ...
      'override' => 'sometimes|required|boolean',
        ...
        if ($success) {
            if (isset($request->returnData)) {
                $returnData = $request->returnData;
            }
            if (isset($request->override)) {
                $override = $request->override;
            }

            $return = self::updateModel($request->data, $model, $returnData, $override, $validatorRule, $fillable, false, (isset($request->with)) ? $request->with : null);
            $success = $return["success"];
            $errorTexts = $return["errorTexts"];
            $returnModels = $return["returnModels"];
        }
    ...
    public static function searchModelByRequest(Request $request, $model, $resource = ExtendedResourceCollection::class)
    {
        $success = true;
        $errorTexts = [];
        $returnModels = collect();
      ...
      'data' => 'required|array',
        ...
        if ($success) {
            ...
            if (ArrayType::isAssociative($request->data)) {
                ...
                if (ArrayType::isMultiDimension($request->data)) {
                    $data = $request->data;
                } else {
                    $data = [$request->data];
                }
      ...
      'data.key' => 'required',
        ...
        if ($success) {
            try {
                $tempModel = new $model;
                if (isset($request->with)) {
                    $tempModel = $tempModel->with($request->with);
                }
                if (isset($request->data['withTrashed']) && $request->data['withTrashed']) {
                    $tempModel = $tempModel->withTrashed();
                }
                if (\is_array($request->data['key'])) {
                    $returnModels = $tempModel->where($request->data['key'])->firstOrFail();
                } else {
                    $returnModels = $tempModel->findOrFail($request->data['key']);
                }
            } catch (\Exception $e) {
                $success = false;
                array_push($errorTexts, ["errorText" => $e->getMessage()]);
